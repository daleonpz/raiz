...
class ReportWriter:
    ...
    def print_table_console(
        data: List[List[str]],
        title: str = "Report",
        with_title: bool = True,
        cell_styles: Optional[dict] = {},
        row_style_cb: Optional[
            Callable[[List[str], int], Union[str, Style, None]]
        ] = None,
    ):
    ...
    def write_console(
        self, domain: str = None, req_type: str = None, detail: bool = False
    ):
        """Pretty-prints the report to the console using Rich."""

        console.print(
            f"[blink][bold green]Traceability Report - {self.report_data['timestamp']}[/blink][/bold green]",
            justify="center",
        )
        ...
        if domain or req_type:
            ...
            if domain:
                coverage = self.report_data["detailed_report"]["domains"].get(
                    domain, {}
                )
            elif req_type:
                coverage = self.report_data["detailed_report"]["types"].get(
                    req_type, {}
                )
        else:
        ...
        self.print_table_console(
            coverage_table, title="Coverage Summary", with_title=True
        )
        ...
        if domain or req_type:
            ...
            for req_id, data in self.report_data["report"].items():
                if (domain and data.get("domain") == domain) or (
                    req_type and data.get("type") == req_type
                ):
        ...
        self.print_table_console(
            ...
            row_style_cb=row_style_callback,
        )
        ...
        if detail and not (domain or req_type):
            ...
            self.print_table_console(
                domain_tables, title="Domain Statistics", with_title=True
            )
            self.print_table_console(
                type_tables, title="Type Statistics", with_title=True
            )
    ...
    def write(
        ...
        if fmt == "console":
            ...
            self.console.print(
                f"[green]Traceability JSON report saved to {output_path}[/green]"
            )
        else:
